...
async def ingest(
    ...
    """Embed and index the knowledge JSON.

    By default this is a no-op when the store is already populated; pass
    ``force=true`` to re-embed and overwrite via Chroma's ``upsert``.
    """
...
async def status(
    ...
    """Report the current chunk count and on-disk Chroma path."""
    return RagStatus(
