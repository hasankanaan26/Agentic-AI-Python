...
class EmployeeLookupTool(BaseTool):
    """Search the (in-memory) Acme Corp employee directory.

    Read-only and TTL-cached — repeated lookups within ``cache_ttl`` seconds
    return the same answer without re-walking the directory.
    """
    ...
    async def run(self, query: str, include_contact: bool = False) -> ToolResult:
        """Search by substring across name/role/department; optionally include contact info.

        Args:
            query: At least 2 characters; matched case-insensitively.
            include_contact: When true, append email and phone to each row.
        """
        if len(query.strip()) < 2:
            return ToolResult.fail("Query must be at least 2 characters.")
        # Cache key reflects both the query and the include_contact flag,
        # because they produce different output strings.
